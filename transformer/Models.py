...
from .Layers import FFTBlock_StyleSpeech_EcapaTDNN
...
class Encoder_StyleSpeech(nn.Module):
    ...
    def forward_prosody(self, src_seq, style_vector, mask, prosody, return_attns=False):
        batch_size, max_len = src_seq.shape[0], src_seq.shape[1]
        ...
        if not self.training and src_seq.shape[1] > self.max_seq_len:
            ...
            position_embedded = self.position_enc[:, :max_len, :].expand(batch_size, -1, -1)
        prosody = prosody.unsqueeze(1)
        enc_output = src_embedded + position_embedded
        pdb.set_trace()
        enc_output = enc_output + prosody
        enc_slf_attn_list = []
        for enc_layer in self.layer_stack:
            enc_output, enc_slf_attn = enc_layer(
                enc_output, style_vector, mask=mask, slf_attn_mask=slf_attn_mask
    ...
    def forward_llm(self, src_seq, style_vector, mask, return_attns=False):
        batch_size, max_len = src_seq.shape[0], src_seq.shape[1]
...
class Encoder_StyleSpeech_EcapaTDNN(nn.Module):
    """ Encoder """

    def __init__(self, config):
        super(Encoder_StyleSpeech_EcapaTDNN, self).__init__()
        ...
        self.layer_stack = nn.ModuleList(
            [
                FFTBlock_StyleSpeech_EcapaTDNN(
                    d_model, n_head, d_k, d_v, d_inner, kernel_size, style_dim, dropout=dropout
                )
                for _ in range(n_layers)
            ]
...
class Decoder_StyleSpeech_EcapaTDNN(nn.Module):
    """ Decoder """

    def __init__(self, config):
        super(Decoder_StyleSpeech_EcapaTDNN, self).__init__()
