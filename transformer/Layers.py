...
from .SubLayers import StyleAdaptiveLayerNorm_EcapaTDNN
import pdb
...
class FFTBlock_StyleSpeech_EcapaTDNN(torch.nn.Module):
    """FFT Block"""

    def __init__(self, d_model, n_head, d_k, d_v, d_inner, kernel_size, style_dim, dropout=0.1):
        super(FFTBlock_StyleSpeech_EcapaTDNN, self).__init__()
        self.slf_attn = MultiHeadAttention(n_head, d_model, d_k, d_v, dropout=dropout)
        self.pos_ffn = PositionwiseFeedForward(
            d_model, d_inner, kernel_size, dropout=dropout
        )
        self.saln_1 = StyleAdaptiveLayerNorm_EcapaTDNN(d_model, style_dim)
