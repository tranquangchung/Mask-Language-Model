...
class StyleAdaptiveLayerNorm_EcapaTDNN(nn.Module):
    def __init__(self, in_channel, style_dim):
        super(StyleAdaptiveLayerNorm_EcapaTDNN, self).__init__()
        self.in_channel = in_channel
        self.norm = nn.LayerNorm(in_channel, elementwise_affine=False)
        ...
        # self.style.affine.bias.data[in_channel:] = 0
        # self.norm_tanh = nn.Tanh()
        self.style_gramma = nn.Sequential(
            AffineLinear(style_dim, in_channel),
            # nn.Tanh(),
        )
        self.style_beta = nn.Sequential(
            AffineLinear(style_dim, in_channel),
            # nn.Tanh(),
        )
    ...
    def forward(self, input, style_code):
        ...
        # return out
        #####
        gamma = self.style_gramma(style_code).unsqueeze(1)
        beta = self.style_beta(style_code).unsqueeze(1)
        out = self.norm(input)
        out = gamma * out + beta
        return out